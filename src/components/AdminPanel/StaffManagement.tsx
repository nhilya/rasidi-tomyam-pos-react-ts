...
import { Plus, Search, Pencil, Trash2, Loader2, ChevronDown, ChevronRight, TriangleAlert, Eye, EyeOff } from 'lucide-react';
import { toast } from 'sonner';
import { useTranslation } from 'react-i18next';
import { useStore } from '@/store';
import { getStaff, getPastStaff, getRoles, createStaff, updateStaff, deleteStaff } from '@/api/staff';
...
const ROLE_COLORS: Record<string, string> = {
  ...
  manager:     'bg-blue-100 text-blue-700 border-blue-200 dark:bg-blue-950/30 dark:text-blue-400 dark:border-blue-900',
  supervisor:  'bg-purple-100 text-purple-700 border-purple-200 dark:bg-purple-950/30 dark:text-purple-400 dark:border-purple-900',
  cashier:     'bg-green-100 text-green-700 border-green-200 dark:bg-green-950/30 dark:text-green-400 dark:border-green-900',
  server:      'bg-amber-100 text-amber-700 border-amber-200 dark:bg-amber-950/30 dark:text-amber-400 dark:border-amber-900',
};
...
function StaffTable({ members, loading, onEdit, onDelete, emptyKey }: StaffTableProps) {
    ...
    <Table>
      ...
      <TableBody>
        {loading ? (
          ...
          members.map(member => {
            const role = member.roles[0] ?? 'server';
            return (
              <TableRow key={member.id}>
                ...
                <TableCell>
                  <Badge variant="outline" className={`capitalize text-[10px] uppercase ${ROLE_COLORS[role] ?? ''}`}>
...
export default function StaffManagement() {
  ...
  const canSeePastStaff = user?.role === 'super_admin' || user?.role === 'boss' || user?.role === 'manager';
  ...
  React.useEffect(() => {
    ...
    getRoles()
      ...
      .catch(() => setAvailableRoles(['boss', 'manager', 'supervisor', 'cashier', 'server']));
  }, []);
  ...
  React.useEffect(() => {
    ...
    getPastStaff()
      ...
      .catch(err => toast.error(err instanceof ApiError ? err.message : 'Failed to load past staff'))
  ...
  const applyFilters = (members: ApiUser[]) =>
    members.filter(s => {
      ...
      const matchRole = roleFilter === 'all' || (s.roles[0] ?? '') === roleFilter;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.error(err instanceof ApiError ? err.message : 'Failed to save staff');
    } finally {
      setSaving(false);
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast.error(err instanceof ApiError ? err.message : 'Failed to delete staff');
    } finally {
      setDeleting(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-3xl font-serif font-bold text-foreground">{t('staff.title')}</h2>
              ...
              <div className="grid gap-4 py-4">
                ...
                <div className="grid gap-2">
                  ...
                  <Input
                    ...
                    onChange={e => setForm({ ...form, phone: rawPhone(e.target.value) })}
                  />
                </div>
                <div className="grid gap-2">
                  <Label>
                    {t('staff.password')}
                    {editingStaff && (
                      <span className="text-xs text-muted-foreground ml-2">({t('staff.passwordHint')})</span>
                    )}
                  </Label>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {availableRoles.map(role => {
          const count = staff.filter((s: ApiUser) => (s.roles[0] ?? '') === role).length;
          ...
          return (
            ...
            >
              {String(t(`login.roles.${role}`, { defaultValue: role }))} ({count})
            </button>
          );
        })}
      </div>
        ...
        <Card className="border-border shadow-sm overflow-hidden">
          ...
          >
            <div className="flex items-center gap-2">
              <span className="font-semibold text-sm text-foreground">{t('staff.pastStaff')}</span>
        ...
        <DialogContent className="sm:max-w-sm text-center">
          <div className="flex flex-col items-center gap-4 py-2">
            ...
            <div className="space-y-1">
              <h3 className="text-lg font-bold text-foreground">{t('staff.confirmDeleteTitle')}</h3>
              <p className="text-sm text-muted-foreground">
                {t('staff.confirmDeleteDesc', { name: deleteTarget?.name ?? '' })}
              </p>
            </div>
